...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      ...
      setUser({ name: data.name, email, is_profile_completed: data.is_profile_completed });
      return data;
    } catch (error) {
  ...
  const googleLogin = async (accessToken) => {
    try {
      ...
      localStorage.setItem('token', data.token);
      setUser({ name: data.name, email: data.email, is_profile_completed: data.is_profile_completed });
      return data;
    } catch (error) {
