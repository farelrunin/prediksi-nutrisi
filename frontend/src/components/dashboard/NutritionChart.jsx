  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-800 border border-white/20 rounded-xl p-4 shadow-xl backdrop-blur-xl">
        <p className="font-semibold text-white mb-2">{`${translations[localStorage.getItem('language') || 'id'].days}: ${label}`}</p>
...
const NutritionChart = ({ data = [] }) => {
  ...
  const dayFormatter = new Intl.DateTimeFormat(language === 'id' ? 'id-ID' : 'en-US', { weekday: 'short' });
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg md:text-xl font-bold text-white flex items-center gap-2">
            ...
            {language === 'id' ? 'Tren Mingguan' : 'Weekly Trend'}
          </h3>
          <p className="text-slate-300 text-sm mt-1">{t.monitorIntake}</p>
        </div>
      ...
      <div className="h-64 md:h-80">
        {hasHistory ? (
          ...
          <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-white/10 bg-slate-950/20 px-6 text-center text-slate-400">
            {language === 'id' ? 'Belum ada riwayat asupan. Tambahkan makanan terlebih dahulu agar tren 7 hari dapat muncul.' : 'No intake history yet. Add food first so the 7-day trend can appear.'}
          </div>
        )}
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
          <div className="text-sm text-blue-400 font-medium mb-1">{language === 'id' ? 'Rata-rata Kalori' : 'Avg. Calories'}</div>
          <div className="text-lg md:text-2xl font-bold text-blue-300">
            {Math.round(averageCalories || 0)} <span className="text-xs font-normal opacity-70">{t.kcal}</span>
          </div>
        </div>
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4">
          <div className="text-sm text-emerald-400 font-medium mb-1">{language === 'id' ? 'Rata-rata Protein' : 'Avg. Protein'}</div>
