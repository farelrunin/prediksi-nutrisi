...
const BottomNavbar = () => {
  ...
  const navItems = [
    ...
    { label: 'Check', to: '/nutri-check', icon: PlusCircle, isPrimary: true },
    { label: 'History', to: '/history', icon: History },
    { label: 'Profile', to: '/profile', icon: User },
  ];

  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[100] px-6 pb-8 pt-4">
      <div className="bg-[var(--bg-card)]/80 backdrop-blur-2xl border border-[var(--border-card)] rounded-[2.5rem] shadow-2xl flex items-center justify-around p-2 relative overflow-hidden group">
        ...
        {/* Glow Effect */}
        <div className="absolute inset-0 bg-gradient-to-t from-[var(--primary-green)]/5 to-transparent opacity-50 pointer-events-none" />
          ...
          return (
            ...
            >
              <div className={`relative transition-all duration-300 ${isActive ? 'scale-110' : 'scale-100'}`}>
                ...
                {isActive && (
                  <div className="absolute -top-1 -right-1 w-1.5 h-1.5 bg-[var(--primary-green)] rounded-full animate-pulse" />
                )}
              </div>
