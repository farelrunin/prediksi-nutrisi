...
const BottomNavbar = () => {
  ...
  if (!user || ['/login', '/register', '/forgot-password', '/onboarding'].includes(location.pathname)) {
    return null;
  }

  const navItems = [
    { label: t.dashboard,  to: '/dashboard',   icon: Home,            ariaLabel: language === 'id' ? 'Dashboard' : 'Dashboard' },
    { label: t.categories, to: '/categories',  icon: LayoutGrid,      ariaLabel: language === 'id' ? 'Kategori' : 'Categories' },
  ...
  return (
    <nav 
      aria-label={language === 'id' ? 'Navigasi bawah' : 'Bottom navigation'} 
      className="lg:hidden fixed bottom-0 left-0 right-0 z-50 flex justify-center px-4 pb-6 pt-2 pointer-events-none"
    >
      <div className="w-full max-w-md bg-[var(--bg-card)]/80 backdrop-blur-2xl border border-[var(--border-card)] rounded-[2.5rem] shadow-2xl flex items-center justify-around p-2 relative overflow-hidden group pointer-events-auto">
