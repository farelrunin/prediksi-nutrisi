...
export const nutritionService = {
  ...
  async reportIncorrectPrediction(imageUrl, predictedClass) {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${API_BASE_URL}/predict/report-incorrect`, { imageUrl, predictedClass }, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      throw new Error('Failed to report incorrect prediction');
    }
  }
};
