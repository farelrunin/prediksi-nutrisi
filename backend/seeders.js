...
const seedData = async () => {
  try {
    ...
    console.log("📂 Reading dataset: fooddataset.csv...");
    const csvPath = path.join(__dirname, 'data', 'processed', 'fooddataset.csv');
    ...
    await new Promise((resolve, reject) => {
      fs.createReadStream(csvPath)
        .pipe(csv())
        .on('data', (row) => {
          const rawName = row.foodname_100g || row['foodname_100g'] || '';
          ...
          for (const cat of categoryMap) {
            if (cat.keywords.some(kw => foodName.includes(kw))) {
              categoryId = cat.id;
              break;
            }
          }
          ...
          foodsToInsert.push({
            food_name_en: rawName || 'Unknown Food',
            food_name_id: rawName || 'Makanan',
